...
SELECT 
    ...
    ROUND(AVG(charge), 2) as avg_charge_rate,
    ROUND(AVG(time), 2) as avg_shift_hours,
    ROUND(AVG(charge * time), 2) as avg_shift_revenue
FROM cleveland_shifts_logs
GROUP BY shift_type
...
--FROM [Booking_logs v1]
--GROUP BY booking_lead_time_bucket
--ORDER BY MIN(lead_time);
...
--WITH booking_cancel_join AS (
    ...
    --FROM booking_logs b
    --LEFT JOIN cancel_logs c ON b.shift_id = c.shift_id AND b.worker_id = c.worker_id
--)
--SELECT 
    --CASE 
...
--FROM booking_cancel_join
--GROUP BY booking_lead_time_category;
...
WITH shift_cancel_join AS (
    SELECT 
        s.shift_id,
        CASE CAST(strftime('%w', s.start) AS INTEGER)
            WHEN 0 THEN 'Sunday'
            WHEN 1 THEN 'Monday'
            WHEN 2 THEN 'Tuesday'
